...
struct EditTodayOverlay: View {
    ...
    init(store: HeadroomStore, onClose: @escaping () -> Void) {
        self.store = store
...
#Preview("Manage commitments") {
    ZStack {
        FBBackground()
        ManageCommitmentsOverlay(store: HeadroomStore(finances: .sample), onClose: {})
    }
...
#Preview("Edit today") {
    ZStack {
        FBBackground()
        EditTodayOverlay(store: HeadroomStore(finances: .sample), onClose: {})
    }
    .preferredColorScheme(.dark)
}
